...
def load_local_eval_dataset(dataset_path: str) -> Dataset:
    """Load a local JSON/JSONL/Parquet evaluation file without Hub metadata."""
    local_path = Path(dataset_path).expanduser()
    if not local_path.exists():
        raise FileNotFoundError(f"Evaluation dataset path does not exist: {local_path}")

    if local_path.is_file():
        files = [local_path]
    else:
        files = sorted(local_path.glob("*.json")) + sorted(local_path.glob("*.jsonl"))
        files += sorted(local_path.glob("*.parquet"))
        files += sorted((local_path / "data").glob("*.parquet"))
    if not files:
        raise FileNotFoundError(f"No JSON/JSONL/Parquet files found under {local_path}")
    ...
    if suffixes <= {".json", ".jsonl"}:
        ...
        for path in files:
            with path.open("r", encoding="utf-8") as handle:
                if path.suffix.lower() == ".jsonl":
                    ...
                    records.extend(payload if isinstance(payload, list) else payload["data"])
        ...
        records = []
        for path in files:
            # Read only scalar fields used by the evaluator.  This avoids the
            # legacy datasets ``List`` metadata in MathArena parquet files.
            schema_names = set(pq.read_schema(path).names)
            required = [name for name in ("problem", "answer", "solution", "id", "problem_idx") if name in schema_names]
            table = pq.read_table(path, columns=required)
            records.extend(table.to_pylist())
        dataset = Dataset.from_list(records)
    else:
        raise ValueError(f"Mixed or unsupported evaluation files under {local_path}: {files}")

    print(f"Loaded local evaluation dataset from {local_path} with {len(dataset)} problems")
    return dataset
...
def evaluate_math500(
    ...
    print(f"Max Concurrent Sequences: {max_num_seqs or 'vLLM default'}")
    print(f"Seed: {seed}")
    print(f"{'='*70}\n")

    print(f"Loading {dataset_name.upper()} dataset...")
    # Prefer an explicitly supplied local file.  This is important for the
    # MathArena parquet exports, whose legacy metadata is not understood by
    ...
    hub_sources = {
        ...
        "hmmt26": ("MathArena/hmmt_feb_2026", "train"),
        "brumo25": ("MathArena/brumo_2025", "train"),
    }
    name = dataset_name.lower()
    if dataset_path:
        dataset = load_local_eval_dataset(dataset_path)
    elif name in hub_sources:
        repo_id, split = hub_sources[name]
        dataset = load_dataset(repo_id, split=split, trust_remote_code=True)
        print(f"Loaded {repo_id} dataset with {len(dataset)} problems")
    else:
        raise ValueError(
            f"Unknown dataset: {dataset_name}. Choose from: {', '.join(hub_sources)}"
        )
    ...
    sampling_params = SamplingParams(
        ...
        # Match the validated OPSD evaluator: stop on both Qwen EOS tokens.
    ...
    for example in dataset:
        # OPSD competition suite fields (MathArena / local JSONL mirrors).
        problem = example["problem"]
        gt_answer = str(example["answer"])
        question_id = example.get("problem_idx", example.get("id", None))
    ...
    if output_file:
        ...
        summary = {
            ...
            "tensor_parallel_size": getattr(
                getattr(llm.llm_engine, "parallel_config", None), "tensor_parallel_size", None
            ),
            "stop_token_ids": stop_token_ids,
            "seed": seed,
            "val_n": val_n,
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="aime25",
        choices=["aime25", "hmmt25", "aime26", "hmmt26", "brumo25"],
        help="Dataset to use for evaluation (default: aime25)",
    )
    parser.add_argument(
        "--dataset_path",
        type=str,
        default=None,
        help="Optional local JSON/JSONL/Parquet file or directory for the selected dataset",
    )
    parser.add_argument(
        "--max_new_tokens",
        type=int,
    ...
    parser.add_argument("--seed", type=int, default=None, help="Optional vLLM engine/request seed")
    parser.add_argument(
        "--stop_token_ids",
        type=int,
        nargs="+",
        default=[151643, 151645],
        help="Token IDs that terminate generation (OPSD-compatible default: 151643 151645)",
    )
    parser.add_argument(
    ...
    print(f"Dataset path: {args.dataset_path or 'Hub/default'}")
    ...
    print(f"Stop token IDs: {args.stop_token_ids}")
    print("=" * 70 + "\n")
